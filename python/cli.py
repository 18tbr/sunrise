...
try:
    ...
    print(
        "---X This tools requires the numpy module, which seems currently unavailable. Please install numpy before tying to use this tool. The recommended syntax to install numpy is :\n$ python3 -m pip install numpy"
    )
    sys.exit(1)
try:
    ...
    print(
        "---X This command line tool is an interface to a python module (more like script actually) called cable_robot. In order to use this tool you must first make sure that this module is available (it currently isn't). The recommended way to do this is to grab the source code cable_robot.py and put it in the same directory as this cli.py file. The source code for cable_robot is available freely on GitHub at [...]."
    )
    sys.exit(1)
...
class CLI(object):
    ...
    def process_freq(self, split_command, cursor):
        if len(split_command) == cursor:
            ...
            self.bprint(
                f"At what pace do you want the robot to move ? That pace can be computed from the step frequency value, which is the amount of steps the robot will perform during a single second. The current value of the step frequency is {int(1/self.time_step)}.",
                1,
            )
            self.bprint(
                f"The syntax to change the step frequency is :\n'>> frequency <step frequency>'\nStep frequency has to be a positive int value and should be reasonably small. In safe mode the programm will refuse arbitrary large values, because the behavior of the robot is undefined at a fast pace."
            )
            return
        else:
            ...
            try:
                ...
                if int_freq <= 0:
                    self.bprint(
                        f"You asked the robot to take {freq} step each second, but negative (zero included) values are not valid in that context. Please input a positive integer instead.",
                        2,
                    )
                    return
                    ...
                    if self.safe:
                        self.bprint(
                            f"Caution, the behavior of the robot is strongly undefined at a high speed. The inputed speed is too high to be accepted when the robot is in safe mode. Please input a step frequency that is smaller than {self.speed_limit} seconds (i.e. {1/self.speed_limit} seconds between each steps). If you know what you are doing you can also try to raise the speed limit, please consult the help manual to see how to do that.\nThe inputed step frequency {int_freq} has been refused, it will remain at its old value {int(1/self.time_step)}",
                            2,
                        )
                    else:
                        self.bprint(
                            f"The inputed speed is above the preset speed limit (of {self.speed_limit} steps per second). However the robot currently is in unsafe mode, hence the value {int_freq} steps per second won't be refused. Please be carefull during your manipulations.",
                            1,
                        )
                ...
                self.bprint(
                    f"The value of the step frequency has to be a positive int, but the tool could not parse {freq} as an int. The correct syntax to set the step frequency is :\n'>> frequency <step frequency>'",
                    2,
                )
                return
    ...
    def process_time(self, split_command, cursor):
        if len(split_command) == cursor:
            # i.e. we have no more arguments available
            self.bprint(
                f"At what pace do you want the robot to move ? That pace is computed from the time step value, which is the amount of time the robot will take to perform an infinitesimal step. The current value of the time step is {self.time_step}.",
                1,
            )
            self.bprint(
                f"The syntax to change the time step is :\n'>> time <time step>'\nTime step has to be a float value and should be reasonably small. In safe mode the programm will refuse arbitrary large values, because the behavior of the robot is undefined at a fast pace."
            )
            return
        else:
            ...
            try:
                ...
                if float_pace <= 0:
                    self.bprint(
                        f"You asked the robot to take a step each {pace} seconds, but negative (zero included) values are not valid in that context. Please input a positive float instead.",
                        2,
                    )
                    return
                    ...
                    if self.safe:
                        self.bprint(
                            f"Caution, the behavior of the robot is strongly undefined at a high speed. The inputed speed is too high to be accepted when the robot is in safe mode. Please input a time step that is bigger than {1/self.speed_limit} seconds (i.e. {self.speed_limit} steps per seconds). If you know what you are doing you can also try to raise the speed limit, please consult the help manual to see how to do that.\nThe inputed time step {float_pace} has been refused, it will remain at its old value {self.time_step}",
                            2,
                        )
                    else:
                        self.bprint(
                            f"The inputed speed is above the preset speed limit (of {self.speed_limit} steps per second). However the robot currently is in unsafe mode, hence the value {float_pace} seconds (i.e. {int(1/float_pace)} steps per second) won't be refused. Please be carefull during your manipulations.",
                            1,
                        )
                ...
                self.bprint(
                    f"The value of the time step has to be a positive float, but the tool could not parse {pace} as a float. The correct syntax to set the time step is :\n'>> time <time step>'",
                    2,
                )
                return
    ...
    def process_auto(self, split_command, cursor):
        ...
        if len(split_command) == cursor:
            ...
            self.bprint(
                "You didn't supply file for the trajectory. Looking for an appropriate file ...",
                1,
            )
            try:
                ...
                self.bprint(
                    "The tool was unable to find any suitable file for the trajectory. Please supply a trajectory in order to use the auto mode.",
                    2,
                )
                self.bprint(
                    "The minimal syntax to supply a trajectory is :\n'>> auto <trajectory_file>'"
                )
                return
        else:
        ...
        if len(split_command) == cursor:
            ...
            if extension == ".npy":
                ...
                self.bprint(
                    "Your file seems to be a numpy array saved as text."
                )
                method = "txt"
            elif extension == ".dat":
                self.bprint(
                    "Your file seems to be a numpy array saved in a plateform dependant way."
                )
                method = "dat"
            elif extension == "":
                ...
                self.bprint(
                    "Your file seems to be numpy array saved in the csv format"
                )
                method = "csv"
        else:
            ...
            if method not in known_methods:
                self.bprint(
                    f"The reading method you specified : {method} is unknown to this tool. Please use one of npy, txt, dat, csv or pickle. Read the help manual in order to see how to use either of these.",
                    2,
                )
                return
        ...
        if method == "npy":
            try:
                ...
                self.bprint(
                    f"The data in the file {file_path} could not be loaded. Consider explicitely telling the tool how to open your file, or using another format if the problem persists.",
                    2,
                )
                self.bprint(
                    f"The minimal syntax to explain how to open a file is :\n'>> auto <trajectory_file> <method>'\n Method should be one of 'pickle', 'npy', 'dat', 'csv', 'txt'. Read the help for more information about those methods."
                )
                return
        elif method == "txt":
            try:
            ...
            try:
                ...
                self.bprint(
                    "The dat file format is deprecated and dangerous as it is not plateform independant. Please use the npy format as an alternative instead.",
                    1,
                )
            except Exception as e:
                # same
            ...
            try:
                ...
                self.bprint(
                    "The pickle module is designed to serialize almost any object python can produce and is not specific to numpy. As such, it is not very efficient as saving / loading numpy arrays and should be avoided. Please use npy instead",
                    1,
                )
            ...
            if topic in self.exit_list:
                ...
                if len(split_command) == cursor:
                    ...
                    if method == "npy":
                        ...
                        self.bprint(
                            f"The reading method {method} is unknown to this tool, please use one of npy, txt, csv, dat or pickle.",
                            1,
                        )
                        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Command line frontend used to command cable driven robots.",
        epilog="This tool and the cable robot library were designed by student of the Ecole des Mines de Paris and all of the source code is available at [...] under the GPL 3 License.",
    )
    ...
    parser.add_argument(
        ...
        choices=[0, 1, 2],
        default=2,
        help="The level of verbosity wanted from this tool. Default is 2, 1 means seing only warning and errors and 0 means only seing errors.",
    )

    parser.add_argument(
        "--unsafe",
        action="store_true",
        help="Control the robot in unsafe mode, only use if you know what you are doing!",
    )

    parser.add_argument(
        "--profile",
        default=None,
        help="A profile file describing the physical characteristics of the cable driven robot you are using. More informations can be found online.",
    )

    parser.add_argument(
        "--version", action="version", version="tool version 0.1"
    )
